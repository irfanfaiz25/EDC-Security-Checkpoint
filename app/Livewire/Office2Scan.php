...
class Office2Scan extends Component
{
    use WithPagination;
    ...
    {
        return view('livewire.office2-scan', [
            'checkpoints' => DataCheckpoint::where('lokasi_cp', 'office lt.2')
                ->where('nama_cp', 'like', "%$this->searchCheckpoint%")
                ->paginate($this->selectedPaginationUser),
            'codes' => DataCheckpoint::where('lokasi_cp', 'office lt.2')->get()
        ]);
    }
    ...
    public function scanCheckpoint()
    {
        ...
        $checkpoint = DataCheckpoint::where('nama_cp', $this->scanOffice2);
        $isCpNameFound = DataCheckpoint::where('nama_cp', $this->scanOffice2)
            ->where('lokasi_cp', 'office lt.2')
            ->exists();
        ...
        if ($isCpNameFound) {
            ...
            if (empty($timeCheck)) {
                ...
                $this->showToast('success', "$this->scanOffice2 berhasil di scan !");

                $this->reset('scanOffice2');
            } else {
                $this->showToast('error', "$this->scanOffice2 sudah di scan !");
