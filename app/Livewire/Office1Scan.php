...
class Office1Scan extends Component
{
    ...
    {
        return view('livewire.office1-scan', [
            'checkpoints' => DataCheckpoint::where('lokasi_cp', 'office lt.1')
                ->where('nama_cp', 'like', "%$this->searchCheckpoint%")
                ->paginate($this->selectedPaginationUser),
            'codes' => DataCheckpoint::where('lokasi_cp', 'office lt.1')->get()
        ]);
    }
    ...
    public function scanCheckpoint()
    {
        ...
        $checkpoint = DataCheckpoint::where('nama_cp', $this->scanOffice1);
        $isCpNameFound = DataCheckpoint::where('nama_cp', $this->scanOffice1)
            ->where('lokasi_cp', 'office lt.1')
            ->exists();
        ...
        if ($isCpNameFound) {
            ...
            if (empty($timeCheck)) {
                ...
                $this->showToast('success', "$this->scanOffice1 berhasil di scan !");

                $this->reset('scanOffice1');
            } else {
                $this->showToast('error', "$this->scanOffice1 sudah di scan !");
