...
class Dmc2Scan extends Component
{
    use WithPagination;
    ...
    {
        return view('livewire.dmc2-scan', [
            'checkpoints' => DataCheckpoint::where('lokasi_cp', 'dmc lt.2')
                ->where('nama_cp', 'like', "%$this->searchCheckpoint%")
                ->paginate($this->selectedPaginationUser),
            'codes' => DataCheckpoint::where('lokasi_cp', 'dmc lt.2')->get()
        ]);
    }
    ...
    public function scanCheckpoint()
    {
        ...
        $checkpoint = DataCheckpoint::where('nama_cp', $this->scanDmc2);
        $isCpNameFound = DataCheckpoint::where('nama_cp', $this->scanDmc2)
            ->where('lokasi_cp', 'dmc lt.2')
            ->exists();
        ...
        if ($isCpNameFound) {
            ...
            if (empty($timeCheck)) {
                ...
                $this->showToast('success', "$this->scanDmc2 berhasil di scan !");

                $this->reset('scanDmc2');
            } else {
                $this->showToast('error', "$this->scanDmc2 sudah di scan !");
