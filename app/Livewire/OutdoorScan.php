...
class OutdoorScan extends Component
{
    use WithPagination;
    ...
    {
        return view('livewire.outdoor-scan', [
            'checkpoints' => DataCheckpoint::where('lokasi_cp', 'outdoor')
                ->where('nama_cp', 'like', "%$this->searchCheckpoint%")
                ->paginate($this->selectedPaginationUser),
            'codes' => DataCheckpoint::where('lokasi_cp', 'outdoor')->get()
        ]);
    }
    ...
    public function scanCheckpoint()
    {
        ...
        $checkpoint = DataCheckpoint::where('nama_cp', $this->scanOutdoor);
        $isCpNameFound = DataCheckpoint::where('nama_cp', $this->scanOutdoor)
            ->where('lokasi_cp', 'outdoor')
            ->exists();
        ...
        if ($isCpNameFound) {
            ...
            if (empty($timeCheck)) {
                ...
                $this->showToast('success', "$this->scanOutdoor berhasil di scan !");

                $this->reset('scanOutdoor');
            } else {
                $this->showToast('error', "$this->scanOutdoor sudah di scan !");
